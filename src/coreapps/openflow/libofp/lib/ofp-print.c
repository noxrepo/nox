...
#ifdef VGW_JINDYLIU
static void
ofp_print_gw_fdb_info(struct ds *string, const struct ofp_header *oh)
{
    const struct tenxt_gw_fdb *msg = ofpmsg_body(oh);
   
    (msg->opt == FDB_ADD) ? ds_put_format(string,"add gw fdb info :") :
                                           ds_put_format(string,"delete gw fdb info :");
    ds_put_format(string, " inner_mac="ETH_ADDR_FMT"", ETH_ADDR_ARGS(msg->inner_mac));
    ds_put_format(string, ",inner_ip="IP_FMT"", IP_ARGS(msg->inner_ip));
    ds_put_format(string, ",outer_ip="IP_FMT"", IP_ARGS(msg->outer_ip));    
    ds_put_format(string, ",vni= %d", ntohl(msg->vni));
}
#endif /* VGW_JINDYLIU */
                ...
                struct ds *string, int verbosity)
{
    ...
    case OFPTYPE_TENXT_GW_FDBS_CFG:
        ofp_print_gw_fdb_info(string, msg);
        break;
#endif /* VGW_JINDYLIU */

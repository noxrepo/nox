...
#ifdef VGW_JINDYLIU
        } else if (hdrs->vendor == TENXT_VENDOR_ID) {
            /* Get tencent message subtype (TENXT_*). */
            const struct tencent_header *nh;

            if (length < sizeof *nh) {
                return OFPERR_OFPBRC_BAD_LEN;
            }
            nh = (const struct tencent_header *) oh;
            hdrs->subtype = ntohl(nh->subtype);        
#endif /*VGW_JINDYLIU*/           
        } else {
...
ofphdrs_len(const struct ofphdrs *hdrs)
{
    if (hdrs->type == OFPT_VENDOR) {
#ifdef VGW_JINDYLIU
        /* Actually, the definition of tencent_header is the same with
            * nicira_header currently, we implement our own version here
            * just in case we need redefine tencent_header to a different 
            * structure in the future and don't want to leave alone the
            * potential bug here. */
        if(hdrs->vendor == TENXT_VENDOR_ID) {
            return sizeof(struct tencent_header);
        }else
#endif /* VGW_JINDYLIU */
             ...
             size_t extra_tailroom, struct ofpbuf *buf)
{
    ...
    if (hdrs->type == OFPT_VENDOR) {
#ifdef VGW_JINDYLIU
        /* We can use the orignal code code to assemble tencent_header,
            * but to be more readable, we implement our own version.*/
        if(hdrs->vendor == TENXT_VENDOR_ID) {
            struct tencent_header *th = buf->l2;
            th->vendor = htonl(hdrs->vendor);
            th->subtype = htonl(hdrs->subtype);         
        }else {
#endif /* VGW_JINDYLIU */

...
#ifdef VGW_JINDYLIU
void
ofproto_set_vgw_vxlanif(struct ofproto *p, const char *vgw_vxlanif)
{
    char * old_vxlanif = p->vgw_vxlanif ;
    
    if(old_vxlanif 
        && vgw_vxlanif 
        && (strncmp(old_vxlanif, vgw_vxlanif, 32) !=0)){
        /* Force all fdb about old  vxlanif */
        ofproto_vgw_flush_fdb_entries(p);
    }else if(old_vxlanif && (vgw_vxlanif != NULL) ){
        ofproto_vgw_flush_fdb_entries(p);
    }
    ...
    p->vgw_vxlanif = vgw_vxlanif ? xstrdup(vgw_vxlanif) : NULL;

}

void
ofproto_set_vgw_peth(struct ofproto *p, const char *vgw_peth)
{
    if(p->vgw_peth){
        free(p->vgw_peth);
    }
    p->vgw_peth = vgw_peth ? xstrdup(vgw_peth) : NULL;
}

#define FDB_CMD_LEN 160
#define BUF_LEN 160

void 
ofproto_vgw_flush_fdb_entries(struct ofproto *p) {
    char command[FDB_CMD_LEN];
    int retval = -1;

    memset(command, 0, sizeof(command));
    
    if(p->vgw_vxlanif){
        snprintf(command, FDB_CMD_LEN, "bridge fdb del dev %s ", p->vgw_vxlanif);

        retval = system(command);
        if(retval != 0) {
            VLOG_ERR("Flush fdb entries for device %s failed", p->vgw_vxlanif);
            exit(0);
        }
    }
}
...
#ifdef VGW_JINDYLIU
/*Number of tries when execute command failed*/
#define MAX_EXECUTE_TRIES 3

static void
ofproto_vgw_send_fdb_reply(struct ofconn *ofconn, 
                                        const struct ofp_header *request, 
                                        int fail_entries, const char *fail_entries_data)
{
    struct ofpbuf *buffer;
    struct ofp_tencent_gw_fdb_rep *rep;

    int len = fail_entries * sizeof(struct tenxt_gw_fdb);

    buffer = ofpraw_alloc_reply(OFPRAW_TENXT_GW_FDBS_REP, request, len);
    rep = (struct ofp_tencent_gw_fdb_rep*)buffer->l2;
    rep->fail_entries = htonl(fail_entries);
    ofpbuf_put(buffer, fail_entries_data, len);

    /* generate and send the reply message */
    ofconn_send_reply(ofconn, buffer);
}

static enum ofperr
handle_tenxt_gw_fdb_info(struct ofconn *ofconn,
                             const struct ofp_header *oh)
{
    struct ofproto *ofproto = ofconn_get_ofproto(ofconn);

    const struct tencent_header *th = (const struct tencent_header*)oh;
    struct tenxt_gw_fdb *entp = NULL;
    unsigned int ent_num = 0;
    int i = 0;
    int tries = 0;
    ...
    char *fail_buf = NULL;
    struct tenxt_gw_fdb *curp = NULL;
    int fail_entries = 0;
    uint32_t opt = 0;
    
    /* get the entry number of fdb entries */
    ent_num = (ntohs(th->header.length) - sizeof(struct tencent_header))
                       / sizeof(struct tenxt_gw_fdb);
    fail_buf = malloc(ent_num * sizeof(struct tenxt_gw_fdb));
    if(fail_buf == NULL) {
        VLOG_ERR("Openflow message(xid=%d):Allocate memory for fdb processing failed!",
                            ((struct ofp_header*)oh)->xid);
        return OFPERR_OFPTEC_MEM_ALLOCATED_FAILED;
    }
    
    /*decode command and excute command*/
    curp = (struct tenxt_gw_fdb *)fail_buf;
    entp = (struct tenxt_gw_fdb*)(th + 1);
     
    for(i = 0; i < ent_num; i++, entp++)
    {
        memset(buffer, 0, sizeof(buffer));
        memset(command, 0, sizeof(command));
        VLOG_DBG("etnry %d: opt=%d, innner_mac="ETH_ADDR_FMT", inner_ip="IP_FMT", dev=%s, dst="IP_FMT" via %s\n", i, entp->opt, 
			ETH_ADDR_ARGS(entp->inner_mac), IP_ARGS(entp->inner_ip), ofproto->vgw_vxlanif, IP_ARGS(entp->outer_ip),  ofproto->vgw_peth);
        if(entp->opt == FDB_ADD) {
            snprintf(command, FDB_CMD_LEN, "bridge fdb add "ETH_ADDR_FMT" inner_ip "IP_FMT" dev %s dst "IP_FMT" via %s",
                    ETH_ADDR_ARGS(entp->inner_mac), IP_ARGS(entp->inner_ip), ofproto->vgw_vxlanif,
                    IP_ARGS(entp->outer_ip), ofproto->vgw_peth);
        }else if(entp->opt == FDB_DEL) {
            snprintf(command, FDB_CMD_LEN, "bridge fdb del "ETH_ADDR_FMT" inner_ip "IP_FMT" dev %s",
                    ETH_ADDR_ARGS(entp->inner_mac), IP_ARGS(entp->inner_ip), ofproto->vgw_vxlanif);		
        }else {
            //unknown operation!
            VLOG_ERR("Openflow message(xid=%d):Receive an unknown fdb command!",
                    ((struct ofp_header*)oh)->xid);
            free(fail_buf);
            return OFPERR_OFPTEC_OPT_UNKNOWN;
        }
try_execute_cmd_again:
        fp = popen(command, "r");
        if(fp == NULL) {
            if(tries < MAX_EXECUTE_TRIES)
            {
                tries++;
                goto try_execute_cmd_again;
            }else {
                tries = 0;
                /* Something seriously wrong with the host, send an error back to controller,
                    * We need human intervention to fix this! */
                VLOG_ERR("Openflow message(xid=%d):Can't open pipe to execute fdb setting command!\n",
					((struct ofp_header*)oh)->xid);
                free(fail_buf);
                return OFPERR_OFPTEC_CMD_EXECUTED_FAILED;
            }
        }
		
        if(fgets(buffer, sizeof(buffer), fp) != NULL) {
            /* There is output to the fdb command!*/
            if(strstr(buffer, "File exists") != NULL) {
                /* the fdb is already set, ignore the error!*/

            }else {
                /* There is error when apply the fdb entry to kernel, we mark this entry and go on.
                  * After finish the works, we send a reply  with the failed entries to the controller.
                  * It is controller's decision to re-try again or pop up warning messages to ask
                  * human intervention. */
                memcpy(curp,  entp, sizeof(struct tenxt_gw_fdb));
                curp++;
                fail_entries++;
            }
        }
    ...
    if(fail_entries > 0) {
        VLOG_WARN("Openflow message(xid=%d):Receive %d entries, failed to set %d entries!\n",
            ((struct ofp_header*)oh)->xid, ent_num, fail_entries);
    }else {
        VLOG_DBG("Openflow message(xid=%d):Receive %d entries, all success!\n",
            ((struct ofp_header*)oh)->xid, ent_num);
    }
    
    /* Make a reply to controller, either success or with failed entries! */
    ofproto_vgw_send_fdb_reply(ofconn, oh, fail_entries, fail_buf);
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    case OFPTYPE_TENXT_GW_FDBS_CFG:
        return handle_tenxt_gw_fdb_info(ofconn, oh);
#endif /* VGW_JINDYLIU */
